...
N = 2048; %length of signal(it is more effective when power of 2)
...
%B.1
 ...
 for k=0:4
     % just design phi(t-kT) for k=0,1,2,4 and a=0,0.5,1
     figure
     plot(tA,phiA,'DisplayName','ö(t)');
     hold on;
     plot(tA+k*T,phiA, 'DisplayName','ö(t-kT)');
 % we compute tA+k*T because we want to shift the signal to the right
